...
$SelectDevis=$cnx->prepare("SELECT * FROM ".$Prefix."devis_facture WHERE id=:id AND hash=:hash");
...
$SelectTransit=$cnx->prepare("SELECT * FROM ".$Prefix."devis_facture_supplement WHERE hash=:hash AND facture=:facture");
$SelectTransit->bindParam(':facture', $Devis->code, PDO::PARAM_STR);
...
$SelectArticle=$cnx->prepare("SELECT * FROM ".$Prefix."devis_facture_article WHERE hash=:hash AND devis=:devis");
...
$SelectClientFacture=$cnx->prepare("SELECT * FROM ".$Prefix."devis_client WHERE hash=:client AND code=:id");
$SelectClientFacture->bindParam(':client', $SessionClient, PDO::PARAM_STR);
$SelectClientFacture->bindParam(':id', $Devis->client, PDO::PARAM_STR);
...
if (isset($_POST['Model2'])) {
    ...
    if ($NumRowTransit==0) {
        $InsertSupplement=$cnx->prepare("INSERT INTO ".$Prefix."devis_facture_supplement (ddp, cn, op, facture, hash) VALUES (:ddp, :cn, :op, :facture, :hash)");
        ...
        $InsertSupplement->bindParam(':facture', $Devis->code, PDO::PARAM_STR);
        $InsertSupplement->execute();        
    }
    else {
        $UpdateSupplement=$cnx->prepare("UPDATE ".$Prefix."devis_facture_supplement SET ddp=:ddp, cn=:cn, op=:op WHERE facture=:facture AND hash=:hash");
        $UpdateSupplement->bindParam(':facture', $Devis->code, PDO::PARAM_STR);
...
if (isset($_POST['Valider'])) {
  ...
  elseif(!preg_match("#[0-9.]#", $Quantite)) {
        ...
        $InsertArticle2=$cnx->prepare("INSERT INTO ".$Prefix."devis_facture_article (reference, description, PU, quantite_box, quantite, poid_brut, poid_net, TVA, marge, devis, hash) VALUES (:reference, :description, :PU, :quantite_box, :quantite, :poid_brut, :poid_net, :TVA, :marge, :devis, :hash)");
...
if (isset($_POST['Ajouter'])) {
  ...
  elseif(!preg_match("#[0-9.]#", $Quantite)) {
    ...
    $InsertArticle=$cnx->prepare("INSERT INTO ".$Prefix."devis_facture_article (reference, description, PU, quantite_box, quantite, poid_brut, poid_net, TVA, marge, devis, hash) VALUES (:reference, :description, :PU, :quantite_box, :quantite, :poid_brut, :poid_net, :TVA, :marge, :devis, :hash)");
    $InsertArticle->bindParam(':reference', $Reference, PDO::PARAM_STR);
    $InsertArticle->bindParam(':description', $Description, PDO::PARAM_STR);
    $InsertArticle->bindParam(':PU', $PU, PDO::PARAM_STR);
...
if (isset($_POST['Activer'])) {
  ...
  $UpdateAcompte=$cnx->prepare("UPDATE ".$Prefix."devis_facture SET acompte=:acompte, acompte_montant=:accompte WHERE hash=:hash AND id=:id");
...
if (isset($_POST['Desactiver'])) {
  ...
  $UpdateAcompte=$cnx->prepare("UPDATE ".$Prefix."devis_facture SET acompte=0 WHERE hash=:hash AND id=:id");
  $UpdateAcompte->bindParam(':id', $Id, PDO::PARAM_STR);
  $UpdateAcompte->bindParam(':hash', $SessionClient, PDO::PARAM_STR);
  $UpdateAcompte->execute();
...
<html>
<head>
<title>Facture - Modification</title>
...
<script>
        function submitFormDate(element) { 
            ...
            req.open("POST", "<?php echo $Home; ?>/lib/script/Facture/date.php?id=<?php echo $Devis->id; ?>", true);
...
<script>
        function submitFormDevise(element) { 
            ...
            req.open("POST", "<?php echo $Home; ?>/lib/script/Facture/devise.php?id=<?php echo $Devis->id; ?>", true);
            req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            //envoi
            req.send(data);  
        }
...
<script>
        function submitFormTaux(element) { 
            ...
            req.open("POST", "<?php echo $Home; ?>/lib/script/Facture/taux.php?id=<?php echo $Devis->id; ?>", true);
            req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            //envoi
            req.send(data);  
        }
...
<script>
        function submitFormRemarque(element) { 
            ...
            req.open("POST", "<?php echo $Home; ?>/lib/script/Facture/remarque.php?id=<?php echo $Devis->id; ?>", true);
            req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            //envoi
            req.send(data);  
        }
...
<a href="<?php echo $Home; ?>/Facture/Visualisation/?id=<?php echo $Devis->id; ?>" target="_blank"><acronym title="Aperçu en PDF"><img src="<?php echo $Home; ?>/lib/img/pdf.png"/></acronym></a></p>

<div id="info"><form name="Form_Date" action="<?php echo $Home; ?>/lib/script/Facture/date2.php?id=<?php echo $Devis->id; ?>" method="POST">
Facture N°: <?php echo $Devis->code; ?><BR />
...
<form name="form_taux" action="<?php echo $Home; ?>/lib/script/Facture/taux2.php?id=<?php echo $Devis->id; ?>" method="POST">
...
<a href="<?php echo $Home; ?>/Client/Modifier/?id=<?php echo $ClientFacture->id; ?>" ><acronym title="Modifier les informations client"><div id="client">
<?php if (($Devis->client!="NULL")||(!empty($Devis->client))) {    
   echo $ClientFacture->civilite." ".$ClientFacture->nom." ".$ClientFacture->prenom."<BR />"
   .$ClientFacture->adresse."<BR />"
   .$ClientFacture->cp.", ".$ClientFacture->ville."<BR />
   Tel : ".$ClientFacture->tel;   
   } ?>
</div></acronym></a>
...
<TR>
    ...
    <?php echo nl2br(stripslashes($Article->description)); ?>
     ...
     <TD>
        <?php echo number_format($PUMarge, 3,".", "")." ".$Devise; ?>
    </TD>
    <?php if (($Param->model=="model21")||($Param->model=="model22")||($Param->model=="model23")||($Param->model=="model31")||($Param->model=="model32")||($Param->model=="model33")) { ?>
    <TD>
    ...
    <TD class="Center">
        <a href="<?php echo $Home; ?>/Facture/Modifier/Modifier/?id=<?php echo $Article->id; ?>&page=<?php echo $_GET['id']; ?>"><acronym title="Modifier"><img src="<?php echo $Home; ?>/lib/img/Modif.png"/></acronym></a>
      <a href="<?php echo $Home; ?>/Facture/Modifier/SupprArticle.php?id=<?php echo $Article->id; ?>&page=<?php echo $_GET['id']; ?>"><acronym title="Supprimer"><img src="<?php echo $Home; ?>/lib/img/Suppr.png"/></a>
  </TD>
</TR>
<?php
$I++;
...
<TR>
  ...
  <TD>     
    <acronym title="Insérer l'article dans la base de donnée"><input class="center" type="checkbox" name="inserer" value="1"/></acronym>
    </TD>
    <TD>
...
<div id="Total">
<?php  $TotalTTC=$TotalTTC-$AccomptE; ?>
...
<?php echo "Total TTC : <b>".number_format($TotalTTC2, 2,".", "")." ".$Devise."</b><br />";  ?>
</div>
...
<form name="Form_Remarque" action="<?php echo $Home; ?>/lib/script/Facture/remarque2.php?id=<?php echo $Devis->id; ?>" method="POST">
