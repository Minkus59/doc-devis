...
$SelectArticle=$cnx->prepare("SELECT * FROM ".$Prefix."devis_facture_article WHERE id=:article AND hash=:client");
...
if (isset($_POST['Valider'])) {
  ...
  elseif(!preg_match("#[0-9.]#", $Quantite)) {
		...
		$UpdateArticle=$cnx->prepare("UPDATE ".$Prefix."devis_facture_article SET description=:description, PU=:PU, TVA=:TVA, quantite_box=:quantite_box, quantite=:quantite, poid_brut=:poid_brut, poid_net=:poid_net, marge=:marge WHERE id=:article AND hash=:client");
		...
		header("location:".$Home."/Facture/Modifier/?id=".$_GET['page']."&erreur=".urlencode($Erreur));
	}
}
