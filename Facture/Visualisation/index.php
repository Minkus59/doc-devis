...
//Traduction
    if ($Param->delai==0) {
        $Delai="30 days from the date of issue of the invoice";
    }
    if ($Param->delai==1) {
        $Delai="45 days end of month";
    }
    if ($Param->delai==2) {
        $Delai="60 days from the date of issue of the invoice";
    }
    if ($Param->delai==3) {
        $Delai="Cash on receipt of invoice";
    }
}
else {
    if ($Param->delai==0) {
        $Delai="30 jours à compter de la date d'émission de la facture";
    }
    if ($Param->delai==1) {
        $Delai="45 jours fin de mois";
    }
    if ($Param->delai==2) {
        $Delai="60 jours à compter de la date d'émission de la facture";
    }
    if ($Param->delai==3) {
        $Delai="Paiement comptant à réception de la facture";
    }
}

$SelectDevis=$cnx->prepare("SELECT * FROM ".$Prefix."devis_facture WHERE id=:id");
...
$SelectSupplement=$cnx->prepare("SELECT * FROM ".$Prefix."devis_facture_supplement WHERE hash=:hash AND facture=:facture");
$SelectSupplement->bindParam(':facture', $Devis->code, PDO::PARAM_STR);
...
if ($Param->delai=="0") { $Param->delai="30 jours à compter de la date d'émission de la facture"; }
if ($Param->delai=="1") { $Param->delai="45 jours fin de mois"; }
if ($Param->delai=="2") { $Param->delai="60 jours à compter de la date d'émission de la facture"; }
if ($Param->delai=="3") { $Param->delai="Paiement comptant à réception de la facture"; }

//------------------------------------------------------

require($_SERVER['DOCUMENT_ROOT']."/Facture/Visualisation/".$Param->model.".php");
...
$pdf->setRemarque($Devis->remarque);
if (($Param->model=="model21")||($Param->model=="model22")||($Param->model=="model23")||($Param->model=="model31")||($Param->model=="model32")||($Param->model=="model33")) {
...
$pdf->setDelai(stripslashes($Delai));
$pdf->setAE($Param->AE);
...
if ($Devis->acompte!=0) {
    $pdf->setNet(number_format($TotalTTC2, 2,".", " ")." ".$Devise);
}
else {
    $pdf->setNet(number_format($TotalTTC, 2,".", " ")." ".$Devise);
}

$pdf->setTalon("Papillon à découper et à joindre à votre réglement : \n \n CLIENT : ".$Devis->client." \n N° FACTURE : ".$Devis->code." \n DATE : ".date("d/m/Y", $Devis->created)." \n MONTANT TTC : ".$TotalTTC." ".$Devise);
$pdf->Bas();
...
if ((isset($_GET['type']))&&($_GET['type']=="mail")) {
  if (isset($_POST['oui'])) {
    ...
    $file_name = $Home."/Facture/Visualisation/".$Devis->code."-".$SessionClient.".pdf";
    ...
    $message.="\n";
    $message.="<html><head><title>".$Param->nom." - FACTURE</title></head><body>".nl2br($Param->messageFacture)."</body></html>";
    $message.="\n\n";
   ...
   if (!mail($ClientDevis->email, $Param->nom." - FACTURE ", $message, $header)) {
        $Erreur="L'e-mail n'a pu être envoyé, vérifiez que vous l'avez entré correctement !</p>";
    } 
    else {   
      unlink($Devis->code."-".$SessionClient.".pdf");
  
      $Erreur="Un E-mail contenant la facture en pièce jointe vient d'être envoyer à ".$ClientDevis->email.".</p>";
      header('Location:'.$Home.'/Facture/?erreur='.urlencode($Erreur));
    } 
  }
  if (isset($_POST['non'])) {
...
<article>
Etes-vous sur de vouloir envoyer cette facture par E-mail à <?php echo $ClientDevis->email; ?> ?
