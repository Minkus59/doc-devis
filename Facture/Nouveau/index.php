...
if (isset($_POST['Valider1'])) {
    ...
    if ((isset($Type))&&($Type=="Nouveau")) { 
        ...
        elseif (strlen($Adresse)<=2) { 
            ...
            $InsertDevis=$cnx->prepare("INSERT INTO ".$Prefix."devis_facture (code, hash, client, created) VALUES (:code, :hash, :client, :temps)");
            $InsertDevis->bindParam(':code', $CodeFacture, PDO::PARAM_STR);
            ...
            $SelectCode=$cnx->prepare("SELECT * FROM ".$Prefix."devis_facture WHERE code=:code AND hash=:client");
            $SelectCode->bindParam(':code', $CodeFacture2, PDO::PARAM_STR);
            $SelectCode->bindParam(':client', $SessionClient, PDO::PARAM_STR);
            $SelectCode->execute();
            $CodeIDFacture=$SelectCode->fetch(PDO::FETCH_OBJ);

            header("location:".$Home."/Facture/Modifier/?id=".$CodeIDFacture->id);
        }
    }
    ...
    if ((isset($Type))&&($Type=="Existant")) { 
      ...
      $InsertDevis=$cnx->prepare("INSERT INTO ".$Prefix."devis_facture (code, hash, client, created) VALUES (:code, :hash, :client, :temps)");
      $InsertDevis->bindParam(':code', $CodeFacture2, PDO::PARAM_STR);
      $InsertDevis->bindParam(':hash', $SessionClient, PDO::PARAM_STR);
