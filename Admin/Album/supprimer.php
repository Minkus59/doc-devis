...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $Select=$cnx->prepare("SELECT (lien) FROM ".$Prefix."neuro_Album WHERE id=:id");
    ...
    $deleteActu=$cnx->prepare("DELETE FROM ".$Prefix."neuro_Album WHERE id=:id");
...
Etes-vous sur de vouloir supprimer cette photo ? </p>
