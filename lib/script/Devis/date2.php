...
if (empty($Date)) {
    echo "<font color='#FF0000'>Une date doit �tre saisie !</font>";
}
...
elseif (strlen($date[2])!=4) {
    echo "<font color='#FF0000'>L'ann�e doit conporter 4 chiffres !</font>";
}
else {
    $Date=mktime(0, 0, 0, $date[1], $date[0], $date[2]);

    $Insert2=$cnx->prepare("UPDATE ".$Prefix."devis_devis SET created=:date WHERE hash=:client AND id=:id");
